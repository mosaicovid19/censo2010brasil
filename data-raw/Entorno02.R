...
Entorno02 <- abrir_base(base = "Entorno02", censo_dir = censo_dir, cores = cores) %>%
  select(Cod_setor, starts_with("V"))

# Injetar data.frame em tabela do DB
dbWriteTable(censodb, "Entorno02", Entorno02, overwrite = TRUE)

# template de doc (Rd)
cat(paste0(sinew::makeOxygen(Entorno02, add_fields = "source"), "\n"), file = "R/Entorno02.R")
